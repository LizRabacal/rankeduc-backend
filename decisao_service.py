...
ARQUIVO_CPC = "CPC_2023.xlsx" # NOVA CONSTANTE
ANO_REF = 2023

FEATURES_COLS = [
    'id_municipio', 'nome_curso', 'taxa_concorrencia',
    'taxa_evasao', 'idd_normalizado', 'igc_normalizado', 'cpc_normalizado' # NOVA FEATURE
]
...
class DecisaoService:
    def __init__(self):
        ...
        # NOVO PESO PARA CPC
        self.regressao_pesos = {'CONCLUSAO': 0.25, 'EVASAO_INVERSA': 0.10, 'IDD': 0.10, 'IGC': 0.40, 'CPC': 0.15}

        self.df_igc, self.df_idd, self.df_cpc = self._carregar_dados_auxiliares() # CARREGA CPC
    ...
    def _carregar_dados_auxiliares(self):
        """Carrega dados IGC, IDD e CPC com limpeza rigorosa nas chaves (Do XLSX)."""
        try:
            # --- IGC ---
            ...
            # --- CPC (NOVO) ---
            df_cpc = pd.read_excel(ARQUIVO_CPC)
            df_cpc.rename(columns={' Código da IES': 'id_ies', ' Código do Curso': 'id_curso', ' CPC (Contínuo)': 'cpc_continuo' }, inplace=True)
            df_cpc['id_ies'] = df_cpc['id_ies'].astype(str).str.replace(r'\.0$', '', regex=True).str.strip()
            df_cpc['id_curso'] = df_cpc['id_curso'].astype(str).str.replace(r'\.0$', '', regex=True).str.strip()
            df_cpc['id_curso'] = df_cpc['id_curso'].apply(lambda x: x.lstrip('0') if x.isdigit() else x)
            df_cpc = df_cpc[['id_ies', 'id_curso', 'cpc_continuo']].drop_duplicates(subset=['id_ies', 'id_curso'])


            logging.info("Dados auxiliares (IGC/IDD/CPC) carregados e chaves limpas.")
            return df_igc, df_idd, df_cpc # Retorna também o CPC
        except Exception as e:
            logging.error(f"Erro no carregamento de IGC/IDD/CPC (XLSX): {e}")
            return pd.DataFrame({'id_ies': [], 'nome_ies': [], 'igc_continuo': []}), \
                   pd.DataFrame({'id_ies': [], 'id_curso': [], 'idd_continuo': []}), \
                   pd.DataFrame({'id_ies': [], 'id_curso': [], 'cpc_continuo': []}) # Retorna CPC vazio

    def _carregar_amostra_para_treinamento(self) -> pd.DataFrame:
        """Carrega amostra real do BigQuery e mescla IGC/IDD/CPC dos XLSX."""
        if BQ_CLIENT is None:
            logging.error("Cliente BigQuery não inicializado. Não é possível carregar amostra.")
            return pd.DataFrame()

        try:
            # (Consulta BigQuery inalterada)
            query_amostra = f"""
                SELECT
            ...
            df = pd.merge(df, self.df_cpc[['id_ies', 'id_curso', 'cpc_continuo']], on=['id_ies', 'id_curso'], how='left') # NOVO MERGE

            df['igc_continuo'] = df['igc_continuo'].fillna(0)
            df['idd_continuo'] = df['idd_continuo'].fillna(0)
            df['cpc_continuo'] = df['cpc_continuo'].fillna(0) # NOVO FILLNA
            df.drop(columns=['nome_ies'], inplace=True, errors='ignore')

            # O FE é executado aqui. O score é calculado aqui para recalcular pesos.
            df = self._executar_feature_engineering(df, is_for_weight_calc=True)
            return df
    ...
    def _calcular_pesos_regressao(self):
        # ATUALIZAÇÃO: Pesos default com CPC
        if self.df_base_treinamento.empty:
            return {'CONCLUSAO': 0.25, 'EVASAO_INVERSA': 0.10, 'IDD': 0.10, 'IGC': 0.40, 'CPC': 0.15}
        ...
        df_reg['x_cpc_norm'] = df_reg['cpc_normalizado'] # NOVO: CPC Normalizado

        # ATUALIZAÇÃO: Incluir CPC na matriz X
        X = df_reg[['x_idd_norm', 'x_igc_norm', 'x_cpc_norm']]
        ...
        try:
            ...
            # ATUALIZAÇÃO: Incluir CPC nos pesos
            pesos_base = {
                'IDD': abs(results.params.get('x_idd_norm', 0.1)),
                'IGC': abs(results.params.get('x_igc_norm', 0.1)),
                'CPC': abs(results.params.get('x_cpc_norm', 0.1)), # NOVO PESO CPC
                'EVASAO_INVERSA': abs(results.params.get('x_idd_norm', 0.1)), # Usando IDD como proxy simples ou melhor:
    ...
    # Adicionada flag is_for_weight_calc para evitar reprocessamento desnecessário na inicialização
    def _executar_feature_engineering(self, df: pd.DataFrame, is_for_weight_calc: bool = False) -> pd.DataFrame:
        if df.empty:
            return df

        df['idd_continuo'] = df['idd_continuo'].fillna(0)
        df['igc_continuo'] = df['igc_continuo'].fillna(0)
        df['cpc_continuo'] = df['cpc_continuo'].fillna(0) # NOVO: Preenche nulos do CPC
        ...
        # Se esta função for chamada apenas para o cálculo de pesos,
        # self.regressao_pesos ainda não terá sido atualizado pelo OLS, então usamos os defaults.
        pesos = self.regressao_pesos

        df['idd_normalizado'] = df['idd_continuo'] / 5.0
        df['igc_normalizado'] = df['igc_continuo'] / 5.0
        df['cpc_normalizado'] = df['cpc_continuo'] / 5.0 # NOVO: Normalização do CPC (escala de 0 a 5)

        # ATUALIZAÇÃO: Incluir CPC no cálculo do score
        df['score_qualidade'] = (
            ...
            (df['cpc_normalizado'] * pesos['CPC']) # NOVO: CPC no Score
        )

        numeric_cols = df.select_dtypes(include=[np.number]).columns
        df[numeric_cols] = df[numeric_cols].replace([np.inf, -np.inf, np.nan], 0).round(4)

        if is_for_weight_calc:
            return df

        # CATEGORIZAÇÃO (Mantido inalterado)
    ...
    def _treinar_modelo(self):
        ...
        X = self.df_base_treinamento[FEATURES_COLS].copy() # FEATURES_COLS JÁ INCLUI CPC

        # (Restante do método _treinar_modelo inalterado)
    ...
    def obter_ranking(self, municipio_id: str, curso_nome: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            df_filtrado = pd.merge(df_filtrado, self.df_cpc[['id_ies', 'id_curso', 'cpc_continuo']], on=['id_ies', 'id_curso'], how='left')

            df_filtrado['igc_continuo'] = df_filtrado['igc_continuo'].fillna(0)
            df_filtrado['idd_continuo'] = df_filtrado['idd_continuo'].fillna(0)
            df_filtrado['cpc_continuo'] = df_filtrado['cpc_continuo'].fillna(0) # NOVO: Preenche nulos do CPC
            ...
            for row in decisoes.itertuples():
                ...
                resultados.append({
                    ...
                    'cpc_continuo': float(row.cpc_continuo), # NOVO: CPC no retorno
    ...
    # (obter_cursos_por_municipio e obter_ranking_api inalterados)
